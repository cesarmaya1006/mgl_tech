    ...
    $(".ver_modal_proyectos").on("click", function () {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                asignarDataTable_ajax('#tabla_proyectos',5,"portrait","Legal","listado de proyectos",true);
    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            $.each(respuesta.proyectos, function (index, proyecto) {
                ...
                respuesta_html +='<div class="image"><img src="' + $('#folder_imagenes_usuario').val() + '/' + proyecto.lider.foto + '" alt="' + proyecto.lider.nombres + ' ' + proyecto.lider.apellidos + '" title="' + proyecto.lider.nombres + ' ' + proyecto.lider.apellidos + '" class="img-circle elevation-2" alt="User Image" style="max-height:50px;width:auto;"></div>';
            ...
            $('#tabla_proyectos').DataTable().destroy();
            $("#tbody_proyectos").html(respuesta_html);
            asignarDataTable_ajax('#tabla_proyectos',5,"portrait","Legal","listado de proyectos",false);
...
function llenarTablaTareas(empleado_id,data_url,data){
    const tareasModal = new bootstrap.Modal(document.getElementById("tareasModal"));
    // -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*- -*-
    var temp_ruta_tareas_gestion = $('#ruta_tareas_gestion').attr("data_url");
    temp_ruta_tareas_gestion = temp_ruta_tareas_gestion.substring(0,temp_ruta_tareas_gestion.length - 1);
    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            $.each(respuesta.tareas, function (index, tarea) {
                respuesta_html += '<tr>';
                respuesta_html +='<td>';
                respuesta_html +='<a href="' + ruta_tareas_gestion + tarea.id + '" class="btn-accion-tabla text-primary" title="Gestionar tarea"><i class="fas fa-eye mr-2"></i></a>';
                respuesta_html +='</td>';
                respuesta_html+='<td style="white-space:nowrap;">' + tarea.id + '</td>';
                respuesta_html+='<td style="white-space:nowrap;">' + tarea.titulo + '</td>';
                respuesta_html+='<td class="text-center" style="white-space:nowrap;">' + tarea.fec_creacion + '</td>';
                respuesta_html+='<td class="text-center" style="white-space:nowrap;">' + tarea.fec_limite + '</td>';
                ...
                respuesta_html+='<td class="text-center" style="white-space:nowrap;">' + progreso + '</td>';
                var hoy             = new Date();
                ...
                respuesta_html+='<td class="text-center d-grid gap-2"><span class="bg-' + bg_estado + ' pl-3 pr-3 w-100" style="font-size: 0.85em;">' + title_estado + '</span></td>';
                respuesta_html += "</tr>";
            });

            $('#tabla_tareas').DataTable().destroy();
            $("#tbody_tareas").html(respuesta_html);
            asignarDataTable_ajax('#tabla_tareas',5,"portrait","Legal","listado de tareas",false);
        },
        error: function () {},
    });
    tareasModal.show();
...
document.addEventListener('DOMContentLoaded', function () {
    const val_count_tareas_activas = parseInt($('#val_count_tareas_activas').val());
    const val_count_tareas_vencidas = parseInt($('#val_count_tareas_vencidas').val());
    ...
    Highcharts.setOptions({colors: ['#0CE73E', '#F02110']});
    ...
    const chart = Highcharts.chart('container', {
        ...
        title: {
            text: "Total de tareas: " + val_count_tareas_activas,
        },
        tooltip: {
            pointFormat:
        ...
        series: [
            {
                ...
                name: "Browser share",
                data: [
                    ["Tareas Vigentes", tareas_vigentes],
